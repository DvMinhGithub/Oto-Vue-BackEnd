...
const adminController = {
  ...
  login: async (req, res) => {
    try {
      ...
      const token = jwt.sign({ idCustomer: user._id }, process.env.ACCESS_TOKEN, {
        expiresIn: '20m'
      })
      const refreshToken = jwt.sign({ idCustomer: user._id }, process.env.REFRESH_TOKEN, { expiresIn: '24h' })
