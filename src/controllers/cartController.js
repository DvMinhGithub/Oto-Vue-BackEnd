...
const cartController = {
  ...
  addToCart: async (req, res) => {
    try {
      ...
      cartData.totalPrice += productInfo.amountPrice
      await cartData.save()
      res.status(201).json({
        success: true,
        data: cartData,
  ...
  updateCart: async (req, res) => {
    try {
      ...
      if (existingProductIndex === -1 && amountProduct !== 0) {
        ...
        if (amountProduct === 0) {
          const removedProduct = cartData.listProduct.splice(existingProductIndex, 1)[0]
          cartData.totalPrice -= removedProduct.amountProduct * productInfo.amountPrice
        } else {
          const existingProduct = cartData.listProduct[existingProductIndex]
          cartData.totalPrice += productInfo.amountPrice * (amountProduct - existingProduct.amountProduct)
          existingProduct.amountProduct = amountProduct
        }
      }
      await cartData.save()
  ...
  removeCart: async (req, res) => {
    try {
      const { idCustomer, idProduct } = req.params
      const cartData = await cartModel.findOne({ idCustomer })
      const productInfo = await carModel.findById(idProduct)
      const existingProductIndex = cartData.listProduct?.findIndex(
